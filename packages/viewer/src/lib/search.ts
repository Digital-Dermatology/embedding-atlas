...
export async function querySearchResultItems(
  ...
  items: { id: any; distance?: number; distance_percentile?: number }[],
  ...
  let id2item = new Map<any, { id: any; distance?: number; distance_percentile?: number }>();
  ...
  let result = Array.from(r).map((x: any): any => {
    let r: Record<string, any> = { id: x.id, distance: id2item.get(x.id)?.distance, distance_percentile: id2item.get(x.id)?.distance_percentile, fields: {} };
    for (let key in x) {
      ...
      try {
        ...
        let truncated = filtered
          .map((neighbor) => ({
            ...
            distance_percentile: typeof neighbor.distance_percentile === "number" ? neighbor.distance_percentile : undefined,
          }))
          .slice(0, limit);
